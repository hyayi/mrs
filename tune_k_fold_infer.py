...
def infer(args):
    ...
    for fold in range(args.fold_num):
        ...
        train_predictions = trainer.predict(model, dataloaders = data_dm.train_infer_dataloader(), ckpt_path=args.model_path)
        val_predictions = trainer.predict(model, dataloaders = data_dm.val_dataloader(), ckpt_path=args.model_path)
        test_predictions = trainer.predict(model, dataloaders = data_dm.test_dataloader(), ckpt_path=args.model_path)
...
if __name__=="__main__" :
    ...
    parser.add_argument("--model_path", action='append')
