...
from dataset1 import Lung3D_ccii_patient_supcon, MRSupcon ,MRSupconMulti
...
from models.ResNet import SupConResNetMulti
...
parser.add_argument('--model_config', '-config', default='D:/study_d/project/brain/code/supcon_mrs/config_multi.yaml', help='save_dir')
...
def main():
    ...
    train_df,val_df,test_df = data_load_multi(args.train_data_path,args.val_data_path,args.test_data_path) ## 데이터 로드
    ...
    target_model = SupConResNetMulti(name='nnUnet', head='mlp', feat_dim=128,clinical_len=len(train_df.columns)-3, **model_config) ## 모델 선언
    ...
    # prepare data
    ## 데이터 셋 설정 
    train_data = MRSupconMulti(data_df=train_df, data_dir=args.data_dir,transforms=train_transforms,mode='train')   
    val_data = MRSupconMulti(data_df=val_df, data_dir=args.data_dir,transforms=val_transfomrs, mode='val')
    ...
    for epoch in range(initial_epoch, initial_epoch + args.epochs):
        ...
        for i, (imgs,clinicals, labels, ID) in enumerate(pbar):
            imgs = torch.cat([imgs[0],imgs[1]],dim=0)
            clinicals = torch.cat([clinicals[0],clinicals[1]],dim=0) #2*bsz,256,256  ## agementation한 이미지를 합쳐서 넣어주는 과정인거 같은데 아마 한개의 batch로 합칠려는 거 같음 
            #imgs = imgs.unsqueeze(1).float().cuda() #2*bsz,1,256,256 ## ct이미지라서 저렇게 한거 같긴한데 의문점은 3d를 사용했는데 왜 차원은 2차원으로 적어둔거지? 
            labels = labels.float().cuda()
            bsz = labels.shape[0]

            features, pred = target_model(imgs,clinicals) #2*bsz,128 #2*bsz,n_class  모델에 이미지를 넣어서 feature contrativeloss용 와 pred #classficatoin를 얻어옴
...
def val1(net, val_loader, epoch):
    ...
    for i, (data, clinical ,label,id) in enumerate(pbar):
        #data = data.unsqueeze(1)
        data = data.float().cuda()
        clinical = clinical.float().cuda()
        label = label.float().cuda()
        _, pred = net(data,clinical)
        pred = F.softmax(pred)
