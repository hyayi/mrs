...
def objective(trial):
    ...
    config['lighthining_model']['model']['params']['hidden_dims_1'] = [trial.suggest_int("1_n_units_l{}".format(i), 4, 512, log=True) for i in range(2)]
    config['lighthining_model']['model']['params']['hidden_dims_2'] = [trial.suggest_int("2_n_units_l{}".format(i), 4, 512, log=True) for i in range(3)]
    config['lighthining_model']['model']['params']['hidden_dims_3'] = [trial.suggest_int("3_n_units_l{}".format(i), 4, 512, log=True) for i in range(1)]
    ...
    config['lighthining_model']['model']['params']['drop_out_rate_3'] = trial.suggest_float("3_drop_out_rate", 0.1, 0.7)
    ...
    for fold in range(args.fold_num):
        ...
        test_metric_list = trainer.test(model,datamodule = data_dm.test_dataloder(), ckpt_path=checkpoint_callback.best_model_path)
